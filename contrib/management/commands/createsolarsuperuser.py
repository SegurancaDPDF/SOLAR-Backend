...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not options['interactive']:
            try:
                ...
                raise CommandError('; '.join(e.args))
        ...
        if username:
            ...
            servidor = None
            comarca_created = None
            with transaction.atomic():
                if not Servidor.objects.filter(usuario__username=username).exists():
                    user = self.UserModel.objects.get(username=username)
                    comarca_padrao, comarca_created = Comarca.objects.get_or_create(pk=1, defaults={'nome': 'CAPITAL'})
                    papel_padrao = Papel.objects.get(nome='Papel: Administrador do Sistema')
                    servidor = Servidor.objects.create(
                        comarca=comarca_padrao,
                        usuario=user,
                        papel=papel_padrao
                    )
                    self.stderr.write("\nCriado Servidor para {}.".format(username))
            if options['verbosity'] >= 1:
                if servidor:
                    self.stdout.write("\nCriado Servidor para {}.".format(username))
                if comarca_created:
                    self.stdout.write("\nCriado Comarca CAPIPAL.")
                self.stdout.write("Solar Superuser criado com sucesso.")
    ...
    def get_input_data(self, field, message, default=None):
        ...
        try:
            ...
            self.stderr.write("Error: %s" % '; '.join(e.args))
            val = None
