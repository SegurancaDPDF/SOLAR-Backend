...
from contrib.models import Servidor, Util, Defensoria
...
def acessar(request):  # funcao de view para acessar um processo
...
def excluir_parte(request, processo_numero, parte_id):  # funcao de view para excluir uma parte do processo
...
def transferir_parte(request):  # funcao de view para transferir uma parte do processo para outro atendimento
...
def excluir_fase(request, fase_id):  # funcao de view para excluir uma fase do processo
...
def excluir_documento_fase(request, documento_id):  # funcao de view para excluir um documento de uma fase do processo
    try:
...
def salvar(request):  # funcao de view para salvar um processo
...
@reversion.create_revision(atomic=False)
@permission_required('processo.add_processo')
def salvar_extra(request):  # funcao de view para salvar um processo extra
    if request.method == 'POST':
...
def salvar_fase(request):  # funcao de view para salvar uma fase do processo
...
def salvar_documento_fase(request):  # funcao de view para salvar um documento de uma fase do processo
    if request.method == 'POST':
...
def salvar_documento_fase_pre(request, fase=None):
    # funcao de view para salvar um documento de uma fase do processo antes da execução do processo de salvar
    if fase is None:
...
def listar_fases(request, processo_numero):  # funcao de view para listar as fases do processo
...
@login_required
def listar_fase_tipo(request):  # funcao de view para listar os tipos de fases do processo
...
def realizar_audiencia(request, audiencia_id):  # funcao de view para realizar uma audiência de um processo
...
@permission_required('processo.change_audiencia')
@reversion.create_revision(atomic=False)
def remanejar_audiencia(request):  # funcao de view para remanejar audiência
...
def listar_audiencias(request):  # funcao de view para listar as audiências dos processos
...
@login_required
@permission_required('processo.view_processo')
def listar(request):  # funcao de view para listar os processos
...
def get_json(request, processo_numero):
    ...
    # filtra os processos com base no número do processo e o grau, se fornecido
    ...
    if processos:
        # se existirem processos com o número e grau fornecidos, selecionamos o primeiro processo
        processo = processos[0]

        # converte o objeto do processo em um dicionário
        ...
        # adiciona os assuntos relacionados ao processo no dicionário
        ...
        # define os tipos de status possíveis do processo
        ...
        # adiciona as partes relacionadas ao processo no dicionário
        ...
        # prepara o objeto JSON de resposta contendo as informações do processo
        eproc_json = {
            'data': datetime.now(),
            'sucesso': True,
            'mensagem': None,
...
def get_json_fase(request, fase_id):
    try:
        # obtem a fase com o ID fornecido
        fase = Fase.objects.get(id=fase_id, ativo=True)
    except ObjectDoesNotExist:
        # retorna um erro JSON se a fase não for encontrada
        return JsonResponse({'error': True})

    # retorna as informações da fase em formato JSON
...
def listar_acao(request):
    ...
    if not cache_data:
        # se não houver dados em cache, obtém a lista de ações ativas e as armazena em cache por 1 mês
        cache_data = Util.json_serialize(Acao.objects.filter(ativo=True).order_by('nome'))
        cache.set(cache_key, cache_data, 60 * 60 * 24 * 30)  # cache 1 mes

    # retorna a lista de ações em formato JSON
    return JsonResponse(cache_data, safe=False)
...
def set_pendentes_relogio(request):
    ...
    if request.GET.get('closed') == 'true':
        # define um tempo de expiração de 5 minutos para a sessão
        request.session['processo_pendente_relogio'] = datetime.now() + timedelta(minutes=5)
    else:
        # define o tempo de expiração para o horário atual
...
def get_pendentes_por_defensor(request, defensor_id):
    ...
    if not cache_data or not cache_data_mes or not cache_data_vara:
        # se não houver dados em cache, obtém os processos pendentes para o defensor com o ID fornecido
        cache_data = []
...
def numero_processos_com_atualizacao_pendentes(request):
    ...
    if request.user.is_authenticated:
        # obtém todos os processos do tipo Processo.TIPO_EPROC que têm um comprimento de número igual a 20,
        # ainda não foram atualizados e estão ativos
        ...
        # conta o total de processos não atualizados e o total de processos em processo de atualizacao
        total_nao_atualizado = processos.count()
        total_atualizando = processos.filter(
            atualizando=True
        ).count()

        # prepara a resposta como um objeto JSON
        ret = JsonResponse(
            data={
        ...
        # retorna um objeto JSON com status 401 (não autorizado) se o usuário não estiver autenticado
        ret = JsonResponse(
            data={
...
def get_partes_processos(request):
    ...
    if request.method == 'POST' and request.is_ajax():
        # obtém o defensor associado ao usuário autenticado e todas as IDs de defensorias em que atua
        defensor = request.user.servidor.defensor
        ids_defensorias = set(
            defensor.atuacoes(vigentes=True).values_list('defensoria_id', flat=True)
        )

        # filtra as partes que correspondem aos critérios especificados
        partes = Parte.objects.filter(
        ...
        for parte in partes:
            # verifica se a parte está atrasada com base no último status sobrestado
            atrasado = (
                True
                if parte.historico
                and parte.historico[0].status == ParteHistoricoSituacao.STATUS_SOBRESTADO
                and parte.historico[0].fim_sobrestamento < hoje
                else False
            )

            # cria um dicionário com informações relevantes da parte
            parte_info = {
                'id': parte.id,
                'situacao_atual': parte.situacao_atual,
                'atrasado': atrasado,
        ...
        # retorna uma resposta JSON contendo as informações das partes correspondentes
...
class VisualizarView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        # obtém o contexto padrão da classe pai e o atualiza com algumas informações adicionais
