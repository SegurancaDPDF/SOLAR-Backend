...
class Processo(models.Model):
    ...
    credencial_mni_cadastro = models.CharField(u'Credencial MNI de Cadastro', max_length=50, null=True, blank=True, default=None)  # noqa: E501
    ...
    def __str__(self):
        return f'{self.numero} - {self.grau}º Grau' if self.numero and self.grau else f'{self.numero} - Extra Judicial' if self.numero and self.grau == 0 else ''  # noqa: E501
...
class Fase(models.Model):
    ...
    ATIVIDADE_RECURSO = 4
    ATIVIDADE_PETICAO_INICIAL = 5
    ATIVIDADE_HABEAS_CORPUS = 6
    ...
    LISTA_ATIVIDADE = (
        ...
        (ATIVIDADE_PETICAO_INICIAL, 'Petição Inicial'),
        (ATIVIDADE_HABEAS_CORPUS, 'Habeas Corpus'),
    )
...
class FaseTipo(AuditoriaAbstractMixin):
    ...
    judicial = models.BooleanField(default=True)
    extrajudicial = models.BooleanField(default=False)
    peticao_inicial = models.BooleanField('Petição Inicial', default=False)
    habeas_corpus = models.BooleanField('Habeas Corpus', default=False)
...
class Distribuicao(AuditoriaAbstractMixin):
    processo = models.ForeignKey('Processo', related_name='+', blank=True, null=True, default=None,
                                 on_delete=models.DO_NOTHING)
    distribuido_defensor = models.ForeignKey('defensor.Defensor', related_name='+', blank=True, null=True, default=None,
                                             editable=False, on_delete=models.DO_NOTHING)
    distribuido_defensoria = models.ForeignKey('contrib.Defensoria', related_name='+', blank=True, null=True,
