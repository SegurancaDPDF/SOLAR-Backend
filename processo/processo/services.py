...
from assistido.models import Filiacao as PessoaFiliacao
...
from atendimento.atendimento.models import Qualificacao, Tarefa
...
from .forms import AtendimentoForm, ProcessoForm, ProcessoParteForm
# Modulos locais
from .models import (
    Acao,
    Assunto,
...
class AvisoService(object):
    ...
    def get_pessoa_assistida(self, aviso):
        ...
        if assistido.exists():
            ...
            if not (cpf or data_nascimento or nome_genitor or nome_genitora):
    ...
    def salvar_distribuicao(self, aviso_numero, defensoria, defensor, eh_redistribuicao=False):
        ...
        from processo.processo.tasks import (
            procapi_cadastrar_preanalise_honorario_aviso
        )
...
class FaseService(object):
    ...
    def set_tipo_fase_processual(self):
        ...
        # Juri
        resposta['juri'] = Fase.objects.filter(
            tipo__juri=True,
            ativo=True
        ).exclude(
        ...
        # Petição Inicial
        resposta['peticao_inicial'] = Fase.objects.filter(
            tipo__peticao_inicial=True,
            ativo=True
        ).exclude(
            atividade=Fase.ATIVIDADE_PETICAO_INICIAL
        ).update(
            atividade=Fase.ATIVIDADE_PETICAO_INICIAL
        )

        # Habeas Corpus
        resposta['habeas_corpus'] = Fase.objects.filter(
            tipo__habeas_corpus=True,
            ativo=True
        ).exclude(
            atividade=Fase.ATIVIDADE_HABEAS_CORPUS
        ).update(
            atividade=Fase.ATIVIDADE_HABEAS_CORPUS
        )
