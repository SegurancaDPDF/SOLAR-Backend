...
from contrib.services import ConvertToMP4Service, ConvertToWEBMService, GedToPDFService
from constance import config

from processo.peticionamento.tasks import enviar_email_confirmacao_protocolo

# Solar
from .models import Aviso, Fase, Manifestacao, Processo, Parte, ManifestacaoDocumento, Acao, Assunto
from assistido.models import Documento as DocumentoOriginalAssistido, \
     DocumentoAssinado as DocumentoAssinadoAssistido
from atendimento.atendimento.models import Documento as DocumentoOriginalAtendimento, \
    DocumentoAssinado as DocumentoAssinadoAtendimento
from .services import AvisoService, FaseService, ProcessoService, ProcessoEventoService, salvar_processo
from procapi_client.models import HistoricoConsultaProcessos, HistoricoConsultaAvisos
...
from thothsigner_client.exceptions import ThothsignerFailed, ThothsignerUnavailable
from contrib.models import Comarca, DefensoriaVara
...
from notificacoes.tasks import (
    notificar_manifestacao_protocolada
)
...
def procapi_cadastrar_novo_processo_signal(self, numero_processo, grau, cpf_defensor=None):
    ...
    for parte in partes['results']:
        ...
        if (parte.get('pessoa').get('documento_principal') is None
                or parte.get('pessoa').get('nome_genitora') is None):
            continue
