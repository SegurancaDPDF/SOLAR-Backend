...
class BuscarView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        comarca = self.request.user.servidor.comarca
        lista_defensores = []
        lista_defensorias = self.request.user.servidor.defensor.defensorias

        lista_todas_defensorias = Defensoria.objects.select_related(
            'comarca'
        ).ativos().filter(comarca=comarca).order_by('comarca__nome')

        lista_todos_defensores = Defensor.objects.select_related(
            'servidor'
        ).ativos().filter(eh_defensor=True, servidor__comarca=comarca).order_by('servidor__nome')
        ...
        # Reiniciar pagina ao clicar no pinel
        filtros['page'] = 1
        ...
        context.update({
            ...
            'lista_todos_defensores': lista_todos_defensores,
