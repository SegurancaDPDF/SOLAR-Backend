    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():
            ...
            campus_estao_vazios = bool(
                not data['sistema_webservice'] and
                not data['comarca'] and
                not data['vara'] and
                not data['paridade'] and
                not data['defensor'] and
                not data['defensoria']
            )
        ...
        pode_sugerir_defensor_defensoria = (
            config.SUGERIR_DEFENSORIA_E_DEFENSOR_NA_DISTRIBUICAO or (defensor or defensoria)
        )
...
class RedistribuirAvisoView(View):
    def post(self, request, *args, **kwargs):
        ...
        defensoria_id = data.get('defensoria')
        defensor_cpf = data.get('defensor')
        defensoria = None
        defensor = None
        if defensoria_id:
        ...
        if defensor_cpf:
            defensor = Defensor.objects.get(servidor__cpf=defensor_cpf)

        # Atualiza aviso, vinculando defensoria e/ou defensor de acordo a redistribuição
        if defensoria or defensor:
            sucesso, resposta = service.salvar_distribuicao(
                aviso,
                defensoria,
                defensor,
