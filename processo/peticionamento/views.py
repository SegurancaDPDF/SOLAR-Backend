...
from processo.processo.forms import BuscarManifestacaoForm, ProcessoPeticionamentoForm
from processo.processo.tasks import procapi_atualizar_manifestacao, procapi_enviar_manifestacao

# Signo
from notificacoes.tasks import (
    notificar_manifestacao_em_analise
)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if tipo == Manifestacao.TIPO_PETICAO_INICIAL:
            ...
            if form_processo.is_valid():
                ...
                if (
                    processo.originario
                    and processo.originario.tipo in [Processo.TIPO_EPROC, Processo.TIPO_FISICO]
                    and processo.originario.vara
                    and processo.originario.vara.codigo_eproc in varas_codigos  # previne que seja selecionado vara de um processo de outro sistema # noqa: E501
                ):
...
class IndexView(TemplateView, SalvarManifestacaoMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        pode_peticionar = (
            pode_editar
            and possui_autor
            and not pessoas_sem_documento
            and False not in requerente_tem_endereco
            and False not in requerido_tem_endereco
            and (
                not documentos_para_assinatura
                or api_sistema.get('deve_enviar_manifestante')
                or (
                    api_sistema.get('metodo_assinatura_documentos') == APISistema.METODO_ASSINATURA_POR_CERTIFICADO_A1
                    and self.request.user.servidor.defensor.eh_defensor
                )
            )
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        elif (sistema.get('metodo_assinatura_documentos') == APISistema.METODO_ASSINATURA_POR_CERTIFICADO_A1
              and self.request.user.servidor.defensor.eh_defensor):
