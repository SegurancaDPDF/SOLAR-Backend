...
# backport de https://github.com/django/django/blob/1.11.10/django/contrib/auth/views.py
...
from django.contrib.auth.forms import (
    AuthenticationForm, PasswordChangeForm, SetPasswordForm, PasswordResetForm
)
from django.contrib.auth.tokens import default_token_generator
from django.contrib.sites.shortcuts import get_current_site
from django.http import HttpResponseRedirect, QueryDict
from django.shortcuts import resolve_url
from contrib.forms import CustomPasswordResetForm
...
from .utils.http import is_safe_url, urlsafe_base64_decode
...
class LoginView(SuccessURLAllowedHostsMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        self.request.session['auditoria_login'] = True
...
class LogoutView(SuccessURLAllowedHostsMixin, TemplateView):
    ...
    @method_decorator(never_cache)
    def dispatch(self, request, *args, **kwargs):
        if 'auditoria_login_id' in request.session:
            HistoricoLogin.objects.filter(id=request.session['auditoria_login_id']).update(logout=datetime.now())
        auth_logout(request)
