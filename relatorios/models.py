...
class Relatorio(AuditoriaAbstractMixin):
    ...
    objects = AuditoriaBaseManager()  # instanciona o manager de audiência base
    ...
    def get_metabase_url(self):  # retorna a url do metabase caso o tipo seja compativel
    ...
    def to_dict(self):  # retorna o objeto relatorio como um objeto
        return {
            'id': self.id,
            'tipo': self.tipo,
            'titulo': self.titulo,
    ...
    def to_json(self):  # retorna o objeto Relatorio como uma string JSON
...
class Local(AuditoriaAbstractMixin):  # model para a entidade local
    ...
    def pode_parametrizar(self):  # verifica se o local pode ser parametrizado
        return self.pagina == Local.PAGINA_RELATORIO_LISTAR


class Relatorios(models.Model):  # model para a entidade relatorios
